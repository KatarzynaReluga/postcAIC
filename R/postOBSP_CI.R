...
postOBSP_CI <- function(X, y,
  ...
  for (i in 1:n) {
    invV_full_list[[i]] <- solve(sig_e_full * diag(n_cluster_units[i]) +
  ...
  for (b in 1:boot)  {
    ...
    for (i in 1:n) {
      Ga_boot_list[[i]] <- sig_u_full_boot[b] * solve(sig_e_full_boot[b] * diag(n_cluster_units[i]) +
  ...
  postOBSP_up = c(mu_hat_sel + 1.96 * rmse_b)
  postOBSP_do = c(mu_hat_sel - 1.96 * rmse_b)
